...
public class ComponentMovementSystem : IGameSystem
{
    ...
    private void UpdateColliderPosition(Entity entity, Vector2 newPosition)
    {
        ...
        // Calculate collider offset (same logic as original)
        if (entity.HasComponent<SpriteComponent>())
        {
            var sprite = entity.GetComponent<SpriteComponent>();
            float colliderWidth = sprite.Sprite.Width * GameConfig.ColliderWidthRatio;
            float colliderHeight = sprite.Sprite.Height * GameConfig.ColliderHeightRatio;
            
            var newColliderBounds = new Rectangle(
                (int)(newPosition.X + (sprite.Sprite.Width - colliderWidth) / 2),
                (int)(newPosition.Y + (sprite.Sprite.Height - colliderHeight) / 2),
                (int)colliderWidth,
                (int)colliderHeight
            );
